...
public class CPU extends Thread implements Hardware {
	...
	private Word[] m; // CPU acessa MEMORIA, guarda referencia 'm' a ela. memoria nao muda. ee sempre
						// a mesma.
	public Memory memory; // CPU acessa MEMORIA, guarda referencia 'm' a ela. memoria nao muda. ee sempre
							// a mesma.
	...
	public void run() { // execucao da CPU supoe que o contexto da CPU, vide acima, esta devidamente
		...
		do {
			...
			if (VM.get().fp.filaProntos.size() == 0) {

				Console.debug("Aguardando processos...");
				try {
					Thread.sleep(5000);
			...
			} else if(VM.get().fp.filaProntos.size() > 0) {
				
				this.setContext(VM.get().fp.filaProntos.peek().allocatedPages, VM.get().fp.filaProntos.peek().pc,VM.get().fp.filaProntos.peek().id, VM.get().fp.filaProntos.peek().reg, VM.get().fp.filaProntos.peek().name);

				Console.debug(" > Executando: " + name);

				while (CLOCK > 0 && interrupt == Interrupt.NONE) {// ciclo de instrucoes. acaba cfe instrucao, veja cada
																	// caso.
					...
					case LDD: // Rd ← [A]
						...
						if ((translate(valor) < 0) || (translate(valor) > 1023)) { // Endereço Invalido -> (ir.p < 0) ||
																					// (ir.p > 1024)
							interrupt = Interrupt.INVALID_ADDRESS;
						} else {
					...
					case STX: // [Rd] ←Rs
						...
						if ((translate(valor) < 0) || (translate(valor) > 1023)) { // Rs está apontando para fora da
																					// memória
...
//         		Demais Instruções
				...
				case INVALID_ADDRESS: // 1
					InterruptHandling.rotinaEnderecoMemoriaInvalido(interrupt, ir.opc); // desvia para a rotina de
																						// Tratamento
		...
		} while (interrupt == Interrupt.NONE || interrupt == null);// FIM DO COMANDO SWITCH FETCH
